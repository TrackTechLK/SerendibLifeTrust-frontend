...
const Section = ({ title }) => {
  return (<div style={{ padding: 10, width: '100%' }}>
...
const ScholarshipCreate = () => (
  <Create>
    <SimpleForm>
      ...
      <RadioButtonGroupInput source="student_detials" defaultValue={'existing'} label='Select whether to create a student or choose existing' choices={[
      ...
      <FormDataConsumer>
        {({ formData, ...rest }) => {
          ...
          if (formData.student_detials == 'existing') {
            return <ReferenceInput source='student' reference='students'>
      ...
      <FormDataConsumer>
        {({ formData, ...rest }) => {
          ...
          if (formData.sponsor_detials == 'existing') {
            return <ReferenceInput source='sponsor' reference='sponsors'>
              <AutocompleteInput optionText='full_name' />
            </ReferenceInput>
          }
          else {
